...
const EventCard = ({ event, index }: EventCardProps) => {
  ...
  const fullDate = event.event_date
    ? new Date(event.event_date).toLocaleDateString("en-US", {
        weekday: "long",
        month: "long",
        day: "numeric",
  ...
  return (
    <>
      <div
        ...
        className="animate-fade-up rounded-2xl overflow-hidden bg-card border border-border shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer"
          ...
          <div className="p-5 space-y-4">
            <DialogHeader className="space-y-1.5 p-0">
              <div className="flex items-start justify-between gap-2">
                ...
                {event.price && (
                  <span className="shrink-0 text-sm font-body font-semibold bg-primary/10 text-primary px-3 py-1 rounded-full">
                    {event.price}
                  </span>
                )}
              </div>
            ...
            <div className="space-y-2.5 bg-muted/50 rounded-xl p-4">
              <div className="flex items-center gap-2 text-foreground">
                ...
                <span className="text-sm font-body font-medium">{fullDate}</span>
              </div>
              {event.event_time && (
                <div className="flex items-center gap-2 text-foreground">
                  <Clock className="w-4 h-4 shrink-0 text-primary" />
                  <span className="text-sm font-body">{event.event_time}</span>
                </div>
              )}
              <div className="flex items-center gap-2 text-foreground">
              ...
              {event.address && (
                <div className="flex items-center gap-2 text-muted-foreground pl-6">
                  <span className="text-xs font-body">{event.address}</span>
                </div>
              )}
            </div>
            ...
            {event.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {event.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs font-body font-medium bg-matcha-light text-accent-foreground px-3 py-1.5 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
